...
def deal_split(income_df, payout_df, rest, zc_id):
    ...
    # 不需要拆分的记录
    income_df_1 = income_df[income_df["id"] < zc_id].copy()
    # 需要拆分的记录
    income_zc = income_df[income_df["id"] == zc_id].copy()
    ...
    # 支出点记录修改支出的金额
    zc_amount = income_df[income_df["id"] == zc_id].show_amount.max() - rest
...
def deal_single_one(income_df, payout_df):
    ...
    # 总收入>=支出时
    if income_df["show_amount"].sum() >= payout_amount:
        # 找到支出点
        for id in income_df.id:
            income_all = (income_all + income_df[income_df["id"] == id].show_amount.sum()).round(2)
            if income_all >= payout_amount:
                zc_id = id
                rest = income_all - payout_amount
                break

        # 拆分后合并
        if rest > 0:
            return deal_split(income_df[income_df["id"]<=zc_id], payout_df, rest, zc_id).\
                append(income_df[income_df["id"]>zc_id], ignore_index=True)
        else:
            return deal_no_split(income_df[income_df["id"]<=zc_id], payout_df).\
                append(income_df[income_df["id"]>zc_id], ignore_index=True)
...
# 生成csv文件
table_to_csv("income.csv", "tmpdb.tmp_income_rest_dtl_01")
table_to_csv("payout.csv", "tmpdb.tmp_user_payout_dtl_01")
print("生成csv文件:payout.csv:", time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))

# 生成dataframe
income_df = pd.read_csv("income.csv")
# 对支出排序
payout_df = pd.read_csv("payout.csv").sort_values(by=['user_id', 'id'], axis=0, ascending=True)
...
# 处理后的结果导入hive
df_to_table(income_df, "result.csv", "tmpdb.tmp_user_result_dtl_01")
